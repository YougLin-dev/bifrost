...
// RequestOverride defines a rule to modify outgoing request parameters.
// When the CEL expression in Match evaluates to true, the Set, Remove, and Defaults
// operations are applied to the serialized JSON request body in that order.
type RequestOverride struct {
	Match    string                 `json:"match"`              // CEL expression (variables: model, request_type). Empty = always match.
	Set      map[string]interface{} `json:"set,omitempty"`      // Force-set params (overwrite if exists)
	Remove   []string               `json:"remove,omitempty"`   // Remove params from request body
	Defaults map[string]interface{} `json:"defaults,omitempty"` // Set params only if not already present
}
